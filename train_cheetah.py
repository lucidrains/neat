...
def train(
    ...
    start_max_episode_len: int = 100,
    end_max_episode_len: int = 1000,
    ...
    # system
    recording_folder: str = './recordings-cheetah',
    recorded_population_folder: str = './recorded-populations-cheetah',
    wandb_online: bool = False
):

    print(f'\nrecordings will be saved to {Path(recording_folder).resolve()}, every {record_every} generations\n')

    run_name = f'neat-cheetah-{pop_size}'
    ...
    wandb.init(project = 'cheetah-neat', mode = 'disabled' if not wandb_online else 'online')
    ...
    def record_agent_(
        ...
        while True:
            actions_to_env = population.single_forward(policy_index, state, sample = False)
            actions_to_env = np.clip(actions_to_env, -1.0, 1.0)
        ...
        video = wandb.Video(
            f'{recording_folder}/cheetah-video-episode-{num_recorded}.mp4',
            format = 'gif'
        )
    ...
    for gen in pbar:
        ...
        for _ in range(num_rollouts_before_evo):
            ...
            while True:
                actions_to_env = population.forward(state, sample = False)
                actions_to_env = np.clip(actions_to_env, -1.0, 1.0)
